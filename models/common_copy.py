...
# csp1_X: X个Res_Unit(Bottleneck)+conv2d+标准conv
class BottleneckCSP(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(BottleneckCSP, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)   # conv1x1(c1,c_)+BN(c_)+SiLU()
        self.cv2 = nn.Conv2d(c1, c_, 1, 1, bias=False)  # conv1x1(c1,c_)
        self.cv3 = nn.Conv2d(c_, c_, 1, 1, bias=False)  # conv1x1(c_,c_)
        self.cv4 = Conv(2 * c_, c2, 1, 1)     # conv1x1(2*c_,c2)
